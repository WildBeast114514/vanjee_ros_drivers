        ...
        bool DecoderVanjee720<T_PointCloud>::decodeMsopPkt720F_FFEE(const uint8_t *pkt, size_t size)
        {
            ...
            for (uint16_t blk = 0; blk < 15; blk++)
            {
                ...
                if (this->split_strategy_->newBlock(azimuth))
                {
                    ...
                    this->first_point_ts_ = pkt_ts - all_points_luminous_moment_720f_[resolution_index][azimuth / resolution * 19];
                    this->last_point_ts_ = this->first_point_ts_ + all_points_luminous_moment_720f_[resolution_index][all_points_luminous_moment_720f_[resolution_index].size()-1];
                    ret = true;
                }
        ...
        bool DecoderVanjee720<T_PointCloud>::decodeMsopPkt720C(const uint8_t *pkt, size_t size)
        {
            ...
            for (uint16_t blk = 0; blk < blacknum; blk++)
            {
                ...
                if (this->split_strategy_->newBlock(azimuth))
                {
                    ...
                    this->first_point_ts_ = pkt_ts - all_points_luminous_moment_720c_[resolution_index][azimuth / resolution * 16];
                    this->last_point_ts_ = this->first_point_ts_ + all_points_luminous_moment_720c_[resolution_index][all_points_luminous_moment_720c_[resolution_index].size()-1];
                    ret = true;
                }
                {
                    double timestamp_point;
