...
namespace vanjee
{
    namespace lidar
    {
        ...
        bool DecoderVanjee720_32<T_PointCloud>::decodeMsopPkt(const uint8_t *pkt, size_t size)
        {
            ...
            double nsec = (packet.difop.time_stamp[0] + (packet.difop.time_stamp[1] << 8) + (packet.difop.time_stamp[2] << 16) + ((packet.difop.time_stamp[3] & 0x0F) << 24)) * 1e-9 * 6.667;
            ...
            for (uint16_t blk = 0; blk < packet.block_num; blk++)
            {
                ...
                if (this->split_strategy_->newBlock(azimuth))
                {
                    ...
                    this->first_point_ts_ = pkt_ts - all_points_luminous_moment_720_32_[resolution_index][azimuth / resolution * 32];
                    this->last_point_ts_ = this->first_point_ts_ + all_points_luminous_moment_720_32_[resolution_index][all_points_luminous_moment_720_32_[resolution_index].size()-1];
                    ret = true;
                }
